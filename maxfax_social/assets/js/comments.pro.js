    ...
    var Comments = {
        ...
        getDefaultOptions: function() {
            return {
                ...
                fileIconURL: '',
                noCommentsIconURL: '',
                iconLikeMy: 'fa fa-heart',
                iconLikeO: 'fa fa-heart-o',
                iconDeleteText: 'fas fa-trash',
                iconSendText: 'fa fa-paper-plane',
                iconEditText: 'fas fa-pen',

                // Strings to be formatted (for example localization)
                textareaPlaceholderText: 'что думаете?',
                newestText: 'Newest',
                oldestText: 'Oldest',
                popularText: 'Popular',
                ...
                sendText: '',//Go
                replyText: '',//Ответить
                editText: '',// Ред   fa fa-edit
                editedText: 'ред',//ред  
                youText: 'You to',
                saveText: '',//Save --edit
                deleteText: '',//Del --edit
                newText: '*', // New
                viewAllRepliesText: 'Ответов __replyCount__ ',
                hideRepliesText: 'Скрыть',
                noCommentsText: '{no comments data}',
                ...
                readOnly: false,
                defaultNavigationSortKey: 'popularity',//newest, oldest, popularity

                // Colors
                highlightColor:		'#2793e6',//#2793e6
                colorIconLikeMy:	'#FF4A4A',
                conterLikeMy:		'#eee',
                ...
                textareaMaxRows: 12,
                maxRepliesVisible: 0,//сколько показывать ответов
                ...
                fieldMappings: {
                    ...
                    fullname: 'name_user',
                    username: 'username',
                    profileURL: 'link_user_profile',
                    upvoteCount: 'upvote_count',
					fileMimeType: 'file_mime_type',
					createdByAdmin: 'created_by_admin',
                    userHasUpvoted: 'user_has_upvoted',
					profilePictureURL: 'avatar_user',
					//profile_picture_url: 'avatar_user',
					createdByCurrentUser: 'creator_owner',//true - если это коммент юзера который просматривает
					timeAgoCreated: 'time_ago_created', // var time  *строка 1684 -- Edited timestamp *1848
					timeAgoModifed: 'time_ago_modified',
					timeUnixCreated: 'time_unix_created',
					timeUnixModifed: 'time_unix_modified',
					timeDataCreatedAll: 'time_data_created_all',
                },
                ...
                pingClicked: function(userId,usernsme=false) {},
        ...
        uploadAttachments: function(files, commentingField) {
            ...
            if(fileCount) {
                ...
                var success = function(commentArray) {
                    ...
                    /**
                     * @todo связать проверку на загрузку файлов с этим условием
                     */
        ...
        sortComments: function (comments, sortKey) {
            ...
            if(sortKey == 'popularity') {
                comments.sort(function(commentA, commentB) {
                    ...
                    if(pointsOfB != pointsOfA) {
						...
						var createdA = commentA.timeUnixCreated;
						var createdB = commentB.timeUnixCreated;
			...
			/*
                comments.sort(function(commentA, commentB) {
					// СОРТИРОВКА ПО ВЕРЕМЕНИ СОЗДАНИЯ - новые вверх
                    var createdA = commentA.timeUnixCreated;
                    var createdB = commentB.timeUnixCreated;
					//console.log(  createdA);
					if(sortKey == 'newest') return createdB - createdA;
                });
			*/	
        },
        ...
        upvoteComment: function(ev) {
            ...
            if(commentModel.userHasUpvoted) {
                newUpvoteCount = --previousUpvoteCount;//like -1
            } else {
                newUpvoteCount = ++previousUpvoteCount;//прирастить like на единицу +1
            }
        ...
        createProfilePictureElement: function(src, userId, username=false, profileURL=false) {
            if(src) {
              //var profilePicture = $('<div/>').css({ 
              var profilePicture = $('<a/>').css({
            ...
            profilePicture.attr('href', profileURL);
        ...
        createCommentingFieldElement: function(parentId, existingCommentId, isMain) {
            ...
            if(existingCommentId) {
                ...
                var username = this.commentsById[existingCommentId].username;
                var profileURL = this.commentsById[existingCommentId].profileURL;
            ...
            // New comment was created
                ...
                var profileURL = this.options.profileURL;
            }
			
            var profilePicture = this.createProfilePictureElement(profilePictureURL, userId, username, profileURL);
            ...
            if(existingCommentId) {
                ...
                var deleteButton = $('<span/>', {
                    'class': 'delete '+ this.options.iconDeleteText,
            ...
            var saveButton = $('<span/>', {
                'class': saveButtonClass + ' save highlight-background '+ this.options.iconSendText,
                text: saveButtonText
            });
            ...
            if(this.options.enablePinging) {
                textarea.textcomplete([{
                    ...
                    template: function(user) {
                        ...
                        var profilePictureEl = self.createProfilePictureElement(user.avatar_user);
                        ...
                        var nameEl = $('<div/>', {
                            'class': 'name',
                        //}).html(user.fullname);
                        }).html(  cropText(user.name_user,14) );

                        var emailEl = $('<div/>', {
                            'class': 'email',
                        }).html(  cropText(user.username,14) );
                    ...
                    replace: function (user) {
                        ...
                        var tag = self.createTagElement('@' + user.name_user, 'ping', user.id_user, {
                ...
                $.fn.textcomplete.Dropdown.prototype.render = function(zippedData) {
					...
					/*
					if($.browser.mobile){
						this.$el.css({'top' : 'auto','left' : '0px', 'position': 'fixed', 'z-index':'100', 'display':'block', 'bottom':'0px', 'width':'100%'});
					}else{
					}
					*/
						var top = parseInt(this.$el.css('top')) + self.options.scrollContainer.scrollTop();
						this.$el.css('top', top+7);
        ...
        createCommentWrapperElement: function(commentModel) {
            ...
            // Profile picture 
            var profilePicture = this.createProfilePictureElement(commentModel.profilePictureURL, commentModel.creator, commentModel.username, commentModel.profileURL);
/*
            // Time
            var time = $('<time/>', {
...
*/
            // Time
            var time = $('<time/>', {
                text: (commentModel.timeAgoCreated),
                'data-original': commentModel.timeDataCreatedAll
            });
...
*/
            ...
            // Edited timestamp
            if(commentModel.timeAgoModifed && commentModel.timeAgoModifed != commentModel.timeAgoCreated) {
                //var editedTime = this.options.timeFormatter(commentModel.modified);
                var editedTime = commentModel.timeAgoModifed;
                var edited = $('<time/>', {
                    'class': 'edited ',
                    text: this.options.textFormatter(this.options.editedText) + ' ' + editedTime,
                    'data-original': commentModel.timeAgoModifed
                });
                content.append(edited);
				//console.log(edited);
            }
            ...
            // Reply
            var reply = $('<button/>', {
                'class': 'action reply   fa fa-reply',
                'type': 'button',
			...
			//условия для показа actions кнопок для пк и мобил
			if($.browser.mobile){
				...
				//code minify : поменял местами кнопки для показа на пк и минимизировал код
				if(this.options.enableUpvoting&&actions.append(upvotes),this.options.enableReplying&&actions.append(reply),commentModel.createdByCurrentUser||this.options.currentUserIsAdmin)if(isAttachment&&this.isAllowedToDelete(commentModel.id)){var deleteButton=$("<button/>",{class:"action delete enabled",text:this.options.textFormatter(this.options.deleteText)});actions.append(deleteButton)}else if(!isAttachment&&this.options.enableEditing){var editButton=$("<button/>",{class:"action edit   fas fa-pen",text:this.options.textFormatter(this.options.editText)});actions.append(editButton)}
			}
        ...
        createCssDeclarations: function() {
            ...
            this.createCss('.go-comments .highlight-background {background: '
                //+ this.options.highlightColor  + ' !important;',
                //+ 'rgba(0,0,0,.001)'  + ' !important;',
                +'}');

            // Font highlight
            this.createCss('.go-comments .highlight-font {color: '
                + this.options.conterLikeMy + ' !important;'
                +'}');
            this.createCss('.go-comments .highlight-font .fa {color: '
                + this.options.colorIconLikeMy + ' !important;'
                +'}');
        ...
        adjustTextareaHeight: function(textarea, focus) {
            ...
            var setRows = function(rows) {
                ...
                textarea.css({'height':height + 'em' , 'font-size':fontSize + 'em'});
            };
        ...
        highlightHashtags: function(commentModel, html) {
            ...
            if(html.indexOf('#') != -1) {
				...
				//teg process регулярные 
                var regex = /(^|\s)#([a-zа-я\d-_]+)/gim;
                //var regex = /(^|\s)#([a-zäöüß\d-_]+)/gim;
                //var regex = /[^0-9\p{L}]+/iu;
