...
$(function(){
	...
	$.ajaxSetup({
		...
		cache: false,
		dataType: "json",
		data: {auth_token: GO.obj.config.auth_token}
	});
	...
	$(window).bind("beforeunload",function() {
		if($("form", GO.obj.modal.selectors.root).data("beforeunload") == "continue") return;
		if($(GO.obj.modal.selectors.root).is(":visible") && GO.fn.form_modal_has_changed()) {
			return GO.fn._s("All the changes that you have made will be lost if you continue.");
		}
	});

	$(window).bind("hashchange", function() {
		// Call edit modal on #edit
		if(window.location.hash=="#edit" && !$(GO.obj.modal.selectors.root).exists()) $("[data-modal=edit]").first().click();
	});
	...
	$(window).on("resize", function(){
		GO.fn.growl.fixPosition();
    GO.fn.modal.fixScrollbars();

		var device = GO.fn.getDeviceName(),
		...
		scrollTimer = setTimeout(function() {
			...
			GO.obj.follow_scroll.set(true);
			GO.obj.follow_scroll.process(true);
		}, 100);

		//GO.fn.window_to_device(); // handled by window event parent
		var new_device = GO.fn.getDeviceName();
		...
		$(".top-bar").css("top", "");
		$(GO.fn.topMenu.vars.menu).css("height", $(window).height());
		...
		GO.fn.list_fluid_width();

		if(width !== $(window).width()) {

			if($("[data-action=top-bar-menu-full]", "#top-bar").hasClass("current")) {
				GO.fn.topMenu.hide(0);
			}

			var cols_fn = function() {
					GO.fn.listing.columnizer(true, 0);
					$(GO.obj.listing.selectors.list_item).show();
				};
			cols_fn();
		}
	...
	$(document).on("click", "[data-trigger]", function(e) {
		...
		if($target.exists()) {
			e.stopPropagation();
			e.preventDefault();
			if(!$target.closest(GO.obj.modal.selectors.root).length) {
				GO.fn.modal.close();
			}
			$target.click();
		} else {
	...
	$("form[method=post]").each(function() {
		if(!$("input[name=auth_token]", this).exists()) {
			$(this).append($('<input>', {type: 'hidden', name: "auth_token", value: GO.obj.config.auth_token}));
		}
	});
	...
	$(document).on("keyup", function(e) {
		$this = $(e.target);
		if(e.keyCode == 27) {
			if($(GO.obj.modal.selectors.root).is(":visible") && !$this.is(":input")) {
				$("[data-action=cancel],[data-action=close-modal]", GO.obj.modal.selectors.root).first().click();
			}
		}
	});
	...
	$(document).on("keyup change blur", ":input[type=password]", function(){
		...
		$("[data-text=password-meter-message]", $parent).removeClass("red-warning").text(password.verdict !== "" ? GO.fn._s(password.verdict) : "");
	...
	$(document).on("click", "[rel=popup-link], .popup-link", function(e){
		...
		if(href.substring(0, 6) == "mailto" && GO.fn.isDevice(["phone", "phablet"])) {
			window.location = href;
			return false;
		}
		GO.fn.popup({href: href});
	});
	...
	$(window).scroll(function(){
		GO.obj.follow_scroll.process(); // todo:optimize
	});
	...
	$(document).on("click", "[data-modal=simple],[data-modal=html]", function(){
		...
		GO.fn.modal.call({template: $target.html(), buttons: false});
	});

	// Prevent modal submit form since we only use the form in the modal to trigger HTML5 validation
	$(document).on("submit", GO.obj.modal.selectors.root + " form", function(e){
	...
	$(document).on("click", "[data-modal=edit],[data-modal=form],[data-confirm]", function(e){
		...
		if($this.is("[data-confirm]")) {
			...
			GO.obj.modal.type = $this.data("modal");
		}
		...
		var args = $this.data("args"),
			...
			ajax = {
				url: $target.data("ajax-url") || (typeof $target.data("is-xhr") !== typeof undefined ? GO.obj.config.json_api : null),
		...
		if($this.is("[data-confirm]")) {
			...
			GO.fn.modal.confirm($.extend(default_options, inline_options));

		} else { // Form/editable

			var default_options = {
					template: $target.html(),
					button_submit: $(this).is("[data-modal=edit]") ? GO.fn._s("Save changes") : GO.fn._s("Submit"),
					confirm: function() {

						var form_modal_has_changed = GO.fn.form_modal_has_changed();

						// Conventional form handling
						var $form = $("form", GO.obj.modal.selectors.root);
						...
						$(":input", GO.obj.modal.selectors.root).each(function(){
							$(this).val($.trim($(this).val()));
						});

						if($this.is("[data-modal=edit]")) {
							// Set the input values before cloning the html
							$target.html($(GO.obj.modal.selectors.body, $(GO.obj.modal.selectors.root).bindFormData()).html().replace(/rel=[\'"]tooltip[\'"]/g, 'rel="template-tooltip"'));
						}
							...
							GO.fn.modal.close(
								function(){
									if(typeof submit_done_msg !== "undefined"){
										GO.fn.growl.expirable(submit_done_msg !== "" ? submit_done_msg : GO.fn._s("Changes saved successfully."));
									}
								}
							);
						}
					...
					cancel: function() {
						...
						if(GO.fn.form_modal_has_changed()) {
							if($(GO.obj.modal.selectors.changes_confirm).exists()) return;
							$(GO.obj.modal.selectors.box, GO.obj.modal.selectors.root).css({transition: "none"}).hide();
							$(GO.obj.modal.selectors.root).append('<div id="'+GO.obj.modal.selectors.changes_confirm.replace("#", "")+'"><div class="content-width"><h2>'+GO.fn._s("All the changes that you have made will be lost if you continue.")+'</h2><div class="'+ GO.obj.modal.selectors.btn_container.replace(".", "") +' margin-bottom-0"><button class="btn btn-input default" data-action="cancel">'+GO.fn._s("Go back to form")+'</button> <span class="btn-alt">'+GO.fn._s("or")+' <a data-action="submit">'+GO.fn._s("continue anyway")+'</a></span></div></div>');
							$(GO.obj.modal.selectors.changes_confirm).css("margin-top", -$(GO.obj.modal.selectors.changes_confirm).outerHeight(true)/2).hide().fadeIn("fast");
						} else {
							GO.fn.modal.close();
			...
			GO.fn.modal.call($.extend(default_options, inline_options));
		}
	...
	$(document).on("click focus", "[data-login-needed], [data-user-logged=must]", function(e) {
		if(!GO.fn.is_user_logged()){
			...
			GO.fn.modal.call({type: "login"});
		}
	});

	// Modal form keydown listener
	$(document).on("keydown", GO.obj.modal.selectors.root + " input", function(e){ // nota: solia ser keyup
		var $this = $(e.target),
		...
		if(key==13 && $("[data-action=submit]", GO.obj.modal.selectors.root).exists() && !$this.is(".prevent-submit")){ // 13 == enter key
			$("[data-action=submit]", GO.obj.modal.selectors.root).click();
		}
	});
	...
	$(document).on("click", "[data-action=top-bar-menu-full]", function() {
		...
		GO.fn.topMenu[hasClass ? "hide" : "show"]();
	});
	...
	$(document).on("click", "[data-action=top-bar-search]", function(){
		...
		if(is_ios() && !$(this).closest(GO.fn.topMenu.vars.menu).exists()) {
	...
	$(document).on("click mouseenter", ".pop-btn", function(e) {
		...
		if(GO.fn.isDevice(["phone", "phablet"]) && (e.type=="mouseenter" || $(this).hasClass("pop-btn-desktop"))) {
			return;
		}
		...
		if($(this).hasClass("disabled") || (($this_click.closest(".current").exists() && !GO.fn.isDevice("phone")) && !$this_click.closest(".pop-btn-show").exists())) {
			return;
		}
		...
		if(GO.fn.isDevice(devices)) {
			var text =  $('.btn-text,.text,.pop-btn-text', $pop_btn).first().text();
			if(typeof text == "undefined" || text == "") {
				text = GO.fn._s("Select");
			}
			if(!$(".pop-box-header", $pop_box).exists()) {
				$pop_box.prepend($('<div/>', {
					"class": 'pop-box-header',
					"html": text + '<span id="closeChevronDownMobile" class="btn-icon fa fa-chevron-down fa-lg"></span></span>'
		...
		if(GO.fn.isDevice(devices)) {
			...
			if($pop_box.is(":visible")) {
				...
				if($this.closest(GO.fn.topMenu.vars.menu).exists()) {
					$(".top-bar").css({transform: "none"});
					$(GO.fn.topMenu.vars.menu).css({
                ...
                setTimeout(function() {
					...
					if($this.closest(GO.fn.topMenu.vars.menu).exists()) {
						...
						$(GO.fn.topMenu.vars.menu).animate({scrollTop: GO.fn.topMenu.vars.scrollTop}, GO.obj.config.animation.normal / 2);
					}
                }, GO.obj.config.animation.normal);
				...
				GO.fn.topMenu.vars.scrollTop = $(GO.fn.topMenu.vars.menu).scrollTop();
                setTimeout(function() {
		...
		if(!GO.fn.isDevice(["laptop", "desktop"])) {
			return;
		}
		var $pop_btn = $(this),
			$pop_box = $(".pop-box", $pop_btn);

		if(!$pop_btn.hasClass("pop-btn-auto") || (GO.fn.isDevice(["phone", "phablet"]) && $pop_btn.hasClass("pop-btn-auto"))) {
			return;
		}

		if(!GO.fn.isDevice(['phone', 'phablet', 'tablet']) && $(this).hasClass("pop-btn-delayed")) {
	...
	$(".pop-btn-delayed").delayedAction(
		{
			delayedAction: function($element) {
				if(GO.fn.isDevice(['phone', 'phablet', 'tablet'])) return;
	...
	if(window.location.hash){
		...
		if($hash_node.exists()) {
			$.each($("[href="+ window.location.hash +"]")[0].attributes, function(){
				GO.obj.tabs.hashdata[this.name] = this.value;
			});
			GO.obj.tabs.hashdata.pushed = "tabs";
	...
	$("a", ".content-tabs").click(function(e) {
		...
		/*if($(this).attr("href") && $(this).attr("href").indexOf("#") === 0) {  // to ->#Hash
			GO.obj.tabs.hashdata = dataTab;
			if(typeof e.originalEvent == "undefined") {
				window.location.hash = GO.obj.tabs.hashdata.href.substring(1);
			}
		} else { // to ->?anything
	...
	$(window).bind("statechange", function(e) {
		...
		if(dataTab && dataTab.pushed == "tabs"){
			GO.fn.show_tab(dataTab["data-tab"]);
		}
	});
	...
	$(document).on("click", "[data-action=load-more]", function(e){
		...
		if(!GO.fn.is_listing() || $(this).closest(GO.obj.listing.selectors.content_listing).is(":hidden") || $(this).closest("#content-listing-template").exists() || GO.obj.listing.calling) return;

		GO.fn.listing.queryString.stock_new();

		// Page hack
		GO.obj.listing.query_string.page = $(GO.obj.listing.selectors.content_listing_visible).data("page");
		GO.obj.listing.query_string.page++;

		// Offset hack
		var offset = $(GO.obj.listing.selectors.content_listing_visible).data("offset");

		if(typeof offset !== "undefined") {
			GO.obj.listing.query_string.offset = offset;
			if(typeof GO.obj.listing.params_hidden == "undefined") {
				GO.obj.listing.params_hidden = {};
			}
			GO.obj.listing.params_hidden.offset = offset;
		} else {
			if(typeof GO.obj.listing.query_string.offset !== "undefined") {
				delete GO.obj.listing.query_string.offset;
			}
			if(GO.obj.listing.params_hidden && typeof GO.obj.listing.params_hidden.offset !== "undefined") {
				delete GO.obj.listing.params_hidden.offset;
			}
		}
	...
	if($(GO.obj.listing.selectors.list_item).length > 0){
		GO.fn.listing.show();
		...
		$(window).scroll(function() {
			...
			if($loadMore.length > 0 && (($(window).scrollTop() + $(window).innerHeight()) > ($(document).height() - 300)) && GO.obj.listing.calling == false) {
			   $loadMore.click();
			}
        });
	...
	$(document).on("click", GO.obj.modal.selectors.root+ " [data-switch]", function(){
		var $this_modal = $(this).closest(GO.obj.modal.selectors.root);
 ...
 */
// pf: get_pf_lang
GO.fn._s = function(string, s){
	var string;
	if(typeof string == "undefined") {
		return string;
	}
	if(typeof GO.obj.l10n !== "undefined" && typeof GO.obj.l10n[string] !== "undefined") {
		string = GO.obj.l10n[string][0];
		if(typeof string == "undefined") {
			string = string;
		}
	} else {
...
GO.fn._n = function(singular, plural, n){
	var string;
	if(typeof GO.obj.l10n !== "undefined" && typeof GO.obj.l10n[singular] !== "undefined") {
		string = GO.obj.l10n[singular][n == 1 ? 0 : 1];
	} else {
 ...
 */
// pf: extend_pf_lang
GO.fn.extend_lang = function(strings){
	$.each(GO.obj.lang_strings, function(i,v){
		if(typeof strings[i] !== "undefined") {
			$.extend(GO.obj.lang_strings[i], strings[i]);
		}
	});
};
...
GO.fn.get_url_var = function(name){
	return GO.fn.get_url_vars()[name];
};

GO.fn.is_user_logged = function() {
	return $("#top-bar-user").exists(); // nota: default version
	// It should use backend conditional
};

GO.fn.generate_random_string = function(len){
...
GO.fn.generate_random_string_hex = function(len){
	if(typeof len == "undefined") len = 5;
    var text = "";
...
GO.fn.htmlEncode = function(value) {
  return $('<div/>').text($.trim(value)).html();
};

GO.fn.htmlDecode = function(value) {
  return $('<div/>').html($.trim(value)).text();
};

GO.fn.nl2br = function(str) {
    var breakTag = '<br>';
    return (str + '').replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1'+ breakTag +'$2');
};

// https://gist.github.com/alexey-bass/1115557
GO.fn.versionCompare = function(left, right) {
 ...
 */
GO.fn.dataURItoBlob = function(dataURI) {
...
GO.fn.clean_facebook_hash = function() {
	if(window.location.hash == "#_=_") {
		window.location.hash = "";
	}
};
GO.fn.clean_facebook_hash();
...
GO.fn.deparam = function(querystring) {
...
jQuery.fn.bindFormData = function() {
	$(":input", this).each(function() {
		var safeVal = GO.fn.htmlEncode($(this).val());
 ...
 */
GO.fn.is_canvas_supported = function(){
 ...
 */
GO.fn.is_validity_supported = function(){
	var i = document.createElement("input");
	return typeof i.validity === "object";
};

GO.fn.getScrollBarWidth = function() {
...
GO.str.ScrollBarWidth = GO.fn.getScrollBarWidth();

/**
 * Updates the notifications button
 */
GO.fn.top_notifications_viewed = function(){
 ...
 */
GO.fn.bindtipTip = function($target, options) {
	...
	$target.each(function(){
		if((typeof $(this).attr("href") !== "undefined" || typeof $(this).data("href") !== "undefined") && GO.fn.isDevice(["phone", "phablet", "tablet"])) {
			return true;
		}
 ...
 */
GO.fn.form_modal_has_changed = function() {
	if($(GO.obj.modal.selectors.root).is(":hidden")) return;
	if(typeof $("html").data("modal-form-values") == typeof undefined) return;

	var data_stored = $("html").data("modal-form-values");
	var data_modal = GO.fn.deparam($(":input:visible", GO.obj.modal.selectors.root).serialize());
...
GO.fn.is_listing = function(){
	return $(GO.obj.listing.selectors.content_listing).exists();
};

GO.fn.is_tabs = function(){
 ...
 * @argument callback fn
 */
GO.fn.scroll = function(target, callback){
...
GO.fn.growl = {
	...
	call: function(options){
		...
		if($(GO.fn.growl.selectors.root).exists()){
			if($(GO.fn.growl.selectors.root).text() == growl_options.message){
				$(GO.fn.growl.selectors.root).shake();
				return;
			}
			$(GO.fn.growl.selectors.root).remove();
		}

		$growl = $('<div id="'+ GO.fn.growl.selectors.root.replace("#", "") +'" class="growl">'+growl_options.message+'<span class="icon icon-close" data-action="close"></span></div>').css(growl_options.css).addClass(growl_options.classes);
		...
		if($(GO.obj.modal.selectors.root).is(":visible")){
			var $modal_box = $(GO.obj.modal.selectors.box, GO.obj.modal.selectors.root);
			$growl.show();
		...
		$(document).on("click", ".growl", function(e){
			if(GO.fn.isDevice(["phone", "phablet"]) || $(e.target).is("[data-action=close]")) {
		...
		if(growl_options.expires > 0){
			...
			this.str.timeout = setTimeout(function(){
				GO.fn.growl.str.timeoutcall = true;
				GO.fn.growl.close();
	...
	expirable: function(msg, time){
		...
		GO.fn.growl.call({message: msg, expires: time});
	},
	...
	close: function(forced, callback){
		var $growl = $(GO.fn.growl.selectors.root);
	...
	fixPosition: function() {
		...
		if(!$growl.exists() || !$(GO.obj.modal.selectors.root).exists()) {
			return;
		}

		if($growl.data("fixedPosition") == "scrollbar" && $(GO.obj.modal.selectors.root).hasScrollBar().vertical) {
			return;
		}

		var	offsetX = {
				modal: $(GO.obj.modal.selectors.box).offset().left,
		...
		$growl.data("fixedPosition", $(GO.obj.modal.selectors.root).hasScrollBar().vertical ? "scrollbar" : "no-scrollbar");
...
GO.obj.modal.$close_buttons = $(GO.obj.modal.selectors.close_buttons, GO.obj.modal.selectors.root);
GO.obj.modal.$submit_button = $(GO.obj.modal.selectors.submit_button, GO.obj.modal.selectors.root);

GO.fn.modal = {

	str: {
		transition: "all " + GO.obj.config.animation.fast + "ms ease"
	},
	...
	call:
		function(options){
			...
			modal_options = {
				...
				buttons: true,
				button_submit: GO.fn._s("Submit"),
				txt_or: GO.fn._s("or"),
				button_cancel: GO.fn._s("cancel"),
			...
			if(typeof options.ajax !== "undefined" && !options.ajax.url && options.ajax.deferred) {
				modal_options.ajax.url = GO.obj.config.json_api;
			}
			...
			if(modal_options.type == "confirm") {
				modal_options.button_submit = GO.fn._s("Confirm");
			}
			...
			$(GO.obj.modal.selectors.root).remove();
            ...
            $("[rel=tooltip]", GO.obj.modal.selectors.root).each(function(){
                GO.fn.bindtipTip(this, {content:$(this).data("title")});
            });

			if($(":button, input[type=submit], input[type=reset]", GO.obj.modal.selectors.root).length > 0) {
				var $form = $("form", GO.obj.modal.selectors.root);
				if($form.exists()) {
					$form.append($($(GO.obj.modal.selectors.btn_container, GO.obj.modal.selectors.root).html()).wrapInner(GO.obj.modal.selectors.btn_container.replace(".", "")));
					$(GO.obj.modal.selectors.btn_container, GO.obj.modal.selectors.root).each(function() {
						if(!$(this).closest("form").exists()) {
							$(this).remove();
						}
					});
				} else {
					$(GO.obj.modal.selectors.box, GO.obj.modal.selectors.root).wrapInner('<form />');
				}
			}

			modal_options.callback();

			$(GO.obj.modal.selectors.box).css({transform: "scale(0.7)", opacity: 0, transition: GO.fn.modal.str.transition});
            $(GO.obj.modal.selectors.root).css({display: "block"});
            setTimeout(function() {
                $(GO.obj.modal.selectors.root).css({opacity: 1});
                $(GO.obj.modal.selectors.box).css({transform: "scale(1)", opacity: 1});
                ...
                setTimeout(function() {
                    ...
                    $("html").data("modal-form-values", GO.fn.deparam($(":input:visible", GO.obj.modal.selectors.root).serialize()));

                }, GO.obj.config.animation.fast);
            }, 1);

            // Bind the modal events
			$(GO.obj.modal.selectors.root).click(function(e){

				var $this = $(e.target),
					_this = this;

				if(GO.obj.modal.locked) {
					return;
				}

				// Changes confirm?
				if($this.closest(GO.obj.modal.selectors.changes_confirm).exists() && ($this.is(GO.obj.modal.selectors.close_buttons) || $this.is(GO.obj.modal.selectors.submit_button))) {

					$(GO.obj.modal.selectors.changes_confirm).remove();

					if($this.is(GO.obj.modal.selectors.close_buttons)) {
						$(GO.obj.modal.selectors.box, _this).fadeIn("fast", function() {
							$(this).css("transition", GO.fn.modal.str.transition);
						});
					} else {
						GO.fn.modal.close();
					}
				...
				// Modal
					...
					if(!$this.closest(".clickable").exists() || $this.is(GO.obj.modal.selectors.close_buttons)) {
						GO.fn.growl.close();
						modal_options.cancel();
					}

					if($this.is(GO.obj.modal.selectors.submit_button)) {
						...
						if($("input, textarea, select", GO.obj.modal.selectors.root).not(":input[type=button], :input[type=submit], :input[type=reset]").length > 0 && !GO.fn.form_modal_has_changed() && !modal_options.forced) {
						...
						if(modal_submit_continue) {
							...
							if(modal_options.ajax.url) {
								...
								var $btn_container = $(GO.obj.modal.selectors.btn_container, GO.obj.modal.selectors.root);
								GO.obj.modal.locked = true;

								$btn_container.first().clone().height($btn_container.height()).html("").addClass("loading").appendTo(GO.obj.modal.selectors.root + " form");
								$btn_container.hide();

								GO.obj.modal.$close_buttons.hide();

								var modal_loading_msg;

								switch(GO.obj.modal.type) {
									case "edit":
										modal_loading_msg = GO.fn._s("Saving");
									break;
									case "confirm":
									case "form":
									default:
										modal_loading_msg = GO.fn._s("Sending");
									break;
								}

								GO.fn.loading.inline($(GO.obj.modal.selectors.btn_container+".loading", GO.obj.modal.selectors.root), {size: "small", message: modal_loading_msg, valign: "center"});

								$(GO.obj.modal.selectors.root).disableForm();

								if(!$.isEmptyObject(GO.obj.modal.form_data) || (typeof options.ajax !== "undefined" && typeof options.ajax.data == "undefined")) {
									modal_options.ajax.data = GO.obj.modal.form_data;
								}
								GO.obj.modal.XHR = $.ajax({
									url: modal_options.ajax.url,
									type: "POST",
									data: modal_options.ajax.data //GO.obj.modal.form_data // $.param ?
								}).complete(function(XHR){

									GO.obj.modal.locked = false;
									...
									if(XHR.status == 200) {
										...
										$(GO.obj.modal.selectors.root).enableForm();
										$(GO.obj.modal.selectors.btn_container+".loading", GO.obj.modal.selectors.root).remove();
										...
										if(typeof modal_options.ajax.deferred !== "undefined" && typeof modal_options.ajax.deferred.error == "function") {
											...
											var message = GO.fn._s("An error occurred. Please try again later.");
											/*
								...
								// No ajax behaviour
								GO.fn.modal.close(modal_options.callback());
							}
	...
	confirm:
		function(options){
			...
			if(typeof options.title == "undefined"){
				options.title = GO.fn._s("Confirm action");
			}
			GO.fn.modal.call(options);
		},
	...
	simple:
		function(options){
			...
			if(typeof options.title == "undefined") options.title = GO.fn._s("information");
			GO.fn.modal.call(options);
		},

	fixScrollbars:
        function() {
            if(!$(GO.obj.modal.selectors.root).exists()) {
                return;
            }
            var $targets = {
            ...
            var properties = {}
            if(GO.str.ScrollBarWidth > 0 && $("html").hasScrollBar().vertical && !$("body").data("overflow-hidden")) {
                properties.padding = GO.str.ScrollBarWidth + "px";
                properties.margin = GO.str.ScrollBarWidth + "px";
            } else {
                properties.padding = "";
                properties.margin = "";
            }
	...
	close:
		function(callback){
			...
			GO.fn.growl.close(true);
			$("[rel=tooltip]", GO.obj.modal.selectors.root).tipTip("hide");
			$(GO.obj.modal.selectors.box).css({transform: "scale(0.5)", opacity: 0});
			$(GO.obj.modal.selectors.root).css({opacity: 0});
            setTimeout(function() {
                if(GO.str.ScrollBarWidth > 0 && $("html").hasScrollBar().vertical) {
...
/**
 * GO FLUID WIDTH FIXER
 * -------------------------------------------------------------------------------------------------
 */
GO.fn.list_fluid_width = function() {
	if(!$("body").is_fluid()) return;

	var $content_listing = $(GO.obj.listing.selectors.content_listing_visible),
		$pad_content_listing = $(GO.obj.listing.selectors.pad_content, $content_listing),
		$list_item = $(GO.obj.listing.selectors.list_item, $content_listing),
		list_item_width = $list_item.outerWidth(true),
		list_item_gutter = $list_item.outerWidth(true) - $list_item.width();

	GO.obj.listing.content_listing_ratio = parseInt(($content_listing.width()+list_item_gutter) / list_item_width);

	if($list_item.length < GO.obj.listing.content_listing_ratio) {
		$pad_content_listing.css("width", "100%");
		return;
	}

	if(GO.fn.isDevice(["tablet", "laptop", "desktop"])) {
	//	$pad_content_listing.width((GO.obj.listing.content_listing_ratio * list_item_width) - list_item_gutter);
	}


	if(GO.obj.follow_scroll.$node.hasClass("position-fixed")) {
		GO.obj.follow_scroll.$node.width($(".content-width").first().width());
	}
...
GO.fn.show_tab = function(tab) {
	...
	if($tab_content_group.exists()){
		...
		var $list_item_target = $(GO.obj.listing.selectors.list_item+":not(.jsly)", $target),
			target_fade = !$target.hasClass("jsly");

		if($target.data("load") == "ajax" && $target.data("empty") !== "true" && !$(GO.obj.listing.selectors.list_item, $target).exists()){
			GO.fn.listing.queryString.stock_load();
			$target.html(GO.obj.listing.template.fill);
			GO.fn.loading.inline($(GO.obj.listing.selectors.content_listing_loading, $target));
			GO.fn.listing.queryString.stock_new();
			GO.fn.listing.ajax();
		} else {
			GO.fn.listing.queryString.stock_current();
			GO.fn.listing.columnizer(false, 0, false);
	...
	if($(GO.obj.listing.selectors.content_listing_visible).data("queued") == true) {
		GO.fn.listing.columnizer(true, 0);
	}
...
GO.obj.listing = {
	...
	current_column: "",
	XHR: {},
	query_string: GO.fn.get_url_vars(),
	calling: false,
...
GO.fn.listing.show = function(response, callback) {

	$content_listing = $("#content-listing-tabs").exists() ? $(GO.obj.listing.selectors.content_listing_visible, "#content-listing-tabs") : $(GO.obj.listing.selectors.content_listing);

	GO.fn.loading.inline(GO.obj.listing.selectors.content_listing_loading);

	$(GO.obj.listing.selectors.list_item+":not(.jsly)", $content_listing).each(function() {

		$(this).imagesLoaded(function(i) {

			var items = GO.obj.listing.selectors.list_item,
				$subjects = $(items+":visible", GO.obj.listing.selectors.content_listing_visible),
				$targets = $(i.elements);

			if((typeof response !== "undefined" && $(response.html).length < GO.obj.config.listing.items_per_page) || $(GO.obj.listing.selectors.list_item, $content_listing).length < GO.obj.config.listing.items_per_page) {
				GO.fn.listing.removeLoader($content_listing);
			}

			if($(GO.obj.listing.selectors.content_listing_pagination, $content_listing).is("[data-type=classic]") || !$("[data-action=load-more]", $content_listing).exists()) {
				$(GO.obj.listing.selectors.content_listing_loading, $content_listing).remove();
			}

			if($subjects.length == 0) {
				$targets.show();
				GO.fn.listing.columnizer(false, 0);
			...
			GO.fn.listing.columnizer(GO.obj.listing.recolumnize, animation_time, $subjects.length == 0);

			$targets.hide();
			GO.obj.listing.recolumnize = false;
			...
			if(GO.fn.isDevice(["laptop", "desktop"])) {
				...
				$targets.each(function() { // too much CPU for this
					...
					$(".image-container", this).imagesLoaded(function(){
						...
						if($subjects.length == 0) {
							if(loadTime > GO.obj.config.animation.normal) {
								$target.fadeIn(GO.obj.config.animation.normal);
							} else {
								$target.show();
							}
						} else {
			...
			GO.obj.listing.calling = false;

			var visible_loading = $(GO.obj.listing.selectors.content_listing_loading, $content_listing).exists() && ($(GO.obj.listing.selectors.content_listing_loading, $content_listing).is_in_viewport());
			if(typeof GO.obj.listing.show_load_more == typeof undefined) {
				GO.obj.listing.show_load_more = visible_loading;
			}

			$(GO.obj.listing.selectors.content_listing_loading, $content_listing)[(visible_loading ? "add" : "remove") + "Class"]("visibility-hidden");
			$(GO.obj.listing.selectors.content_listing_load_more, $content_listing)[(GO.obj.listing.show_load_more ? "show" : "hide")]();
...
GO.fn.listing.removeLoader = function(obj) {

	var remove = [GO.obj.listing.selectors.content_listing_load_more, GO.obj.listing.selectors.content_listing_loading];

	if($(GO.obj.listing.selectors.content_listing_pagination, $content_listing).is("[data-type=endless]")) {
		remove.push(GO.obj.listing.selectors.content_listing_pagination);
	}
...
GO.fn.listing.queryString = {

	// Stock the querystring values from initial load
	stock_load: function() {

		var $content_listing = $(GO.obj.listing.selectors.content_listing_visible),
			params = GO.fn.deparam($content_listing.data("params"));

		GO.obj.listing.params_hidden = typeof $content_listing.data("params-hidden") !== "undefined" ?  GO.fn.deparam($content_listing.data("params-hidden")) : null;

		if(typeof GO.obj.listing.query_string.action == "undefined") {
			GO.obj.listing.query_string.action = $content_listing.data("action") || "list";
		}
		if(typeof GO.obj.listing.query_string.list == "undefined") {
			GO.obj.listing.query_string.list = $content_listing.data("list");
		}
		if(typeof GO.obj.listing.query_string.sort == "undefined") {
			if(typeof params !== "undefined" && typeof params.sort !== "undefined") {
				GO.obj.listing.query_string.sort = params.sort;
			} else {
				GO.obj.listing.query_string.sort = $(":visible"+GO.obj.listing.selectors.sort).data("sort");
			}
		}
		if(typeof GO.obj.listing.query_string.page == "undefined") {
			GO.obj.listing.query_string.page = 1;
		}
		$content_listing.data("page", GO.obj.listing.query_string.page);

		// Stock the real ajaxed hrefs for ajax loads
		$(GO.obj.listing.selectors.content_listing+"[data-load=ajax]").each(function(){

		var $sortable_switch = $("[data-tab="+$(this).attr("id")+"]"+GO.obj.listing.selectors.sort);
		var dataParams = GO.fn.deparam($(this).data("params")),
			dataParamsHidden = GO.fn.deparam($(this).data("params-hidden")),
			params = {
		...
		for(var k in params) {
			if(/action|list|sort|page/.test(k) == false) {
				GO.obj.listing.query_string[k] = params[k];
			}
		}

		if(typeof GO.obj.listing.params_hidden !== typeof undefined) {
			// The additional params setted in data-hidden-params=""
			for(var k in GO.obj.listing.params_hidden) {
				if(/action|list|sort|page/.test(k) == false) {
					GO.obj.listing.query_string[k] = GO.obj.listing.params_hidden[k];
				}
			}
			GO.obj.listing.query_string['params_hidden'] = GO.obj.listing.params_hidden;
			GO.obj.listing.params_hidden['params_hidden'] = null; // Add this key for legacy, params_hidden v3.9.0 intro*
		}
	},
	...
	stock_new: function(){
		...
		if($content_listing.data("offset")) {
			GO.obj.listing.query_string.offset = $content_listing.data("offset");
		} else {
			delete GO.obj.listing.query_string.offset;
		}
...
GO.fn.listing.ajax = function() {

	if(GO.obj.listing.calling == true) {
		return;
	}

	GO.obj.listing.calling = true;

	var $content_listing = $(GO.obj.listing.selectors.content_listing_visible),
		$pad_content_listing = $(GO.obj.listing.selectors.pad_content, $content_listing);

	$(GO.obj.listing.selectors.content_listing_load_more, $content_listing).hide();
	$(GO.obj.listing.selectors.content_listing_loading, $content_listing).removeClass("visibility-hidden").show();

	GO.obj.listing.XHR = $.ajax({
		type: "POST",
		data: $.param($.extend({}, GO.obj.listing.query_string, $.ajaxSettings.data))
	}).complete(function(XHR) {

		var response = XHR.responseJSON;
		var removePagination = function() {
				$(GO.obj.listing.selectors.content_listing_loading+","+GO.obj.listing.selectors.content_listing_pagination+":not([data-visibility=visible])", $content_listing).remove();
			},
			setEmptyTemplate = function() {
				$content_listing.data("empty", "true").html(GO.obj.listing.template.empty);
		...
		if(XHR.readyState == 4 && typeof response !== "undefined") {
			...
			if(XHR.status !== 200) {
				...
				GO.fn.growl.call("Error: "+response_output);
				$content_listing.data("load", "");
			}
			// Empty HTML
			if((typeof response.html == "undefined" || response.html == "") && $(GO.obj.listing.selectors.list_item, $content_listing).length == 0) {
				setEmptyTemplate();
			}
			// End of the line
			if(typeof response.html == "undefined" || response.html == "") {
				removePagination();
				GO.obj.listing.calling = false;
				if(typeof GO.fn.listing_end == "function") {
					GO.fn.listing_end();
				}
				return;
			}
			...
			$content_listing.data({
				"load": "",
				"page": GO.obj.listing.query_string.page
			});

			var url_object = $.extend({}, GO.obj.listing.query_string);
			for(var k in GO.obj.listing.params_hidden) {
			...
			GO.fn.listing.show(response, function() {
				$(GO.obj.listing.selectors.content_listing_loading, $content_listing).addClass("visibility-hidden");
			});
		...
		if(typeof GO.fn.listing.ajax.callback == "function") {
			GO.fn.listing.ajax.callback(XHR);
		}
...
GO.fn.listing.columnizerQueue = function() {
	$(GO.obj.listing.selectors.content_listing+":hidden").data("queued", true);
};

GO.fn.listing.refresh = function(animation_time) {
	GO.fn.listing.columnizer(true, animation_time, false);
	$(GO.obj.listing.selectors.list_item).show();
};

// Go's masonry approach... Just because godlike.
var width = $(window).width();
GO.fn.listing.columnizer = function(forced, animation_time, hard_forced) {
	...
	if(typeof GO.obj.listing.mode == "undefined") forced = true;
	...
	if(typeof animation_time == typeof undefined) var animation_time = GO.obj.config.animation.normal;

	//animation_time = 0;

	var $container = $("#content-listing-tabs").exists() ? $(GO.obj.listing.selectors.content_listing_visible, "#content-listing-tabs") : $(GO.obj.listing.selectors.content_listing),
		$pad_content_listing = $(GO.obj.listing.selectors.pad_content, $container),
		list_mode = "responsive",
		$list_item = $(forced || hard_forced ? GO.obj.listing.selectors.list_item : GO.obj.listing.selectors.list_item+":not(.jsly)", $container);

	$container.addClass("jsly");

	// Get the device columns from global config
	if(typeof GO.obj.config.listing.device_to_columns !== "undefined") {
		device_to_columns = $.extend({}, device_to_columns, GO.obj.config.listing.device_to_columns);
	}
	...
	GO.obj.listing.mode = list_mode;
	GO.obj.listing.device = GO.fn.getDeviceName();

	if(!$list_item.exists()) return;

	if(typeof $container.data("columns") !== "undefined" && !forced && !hard_forced){
		GO.obj.listing.columns = $container.data("columns");
		GO.obj.listing.columns_number = $container.data("columns").length - 1;
		GO.obj.listing.current_column = $container.data("current_column");
	} else {
		var $list_item_1st =  $list_item.first();
		$list_item_1st.css("width", "");
		GO.obj.listing.columns = new Array();
		GO.obj.listing.columns_number = device_to_columns[GO.fn.getDeviceName()];
		for(i=0; i<GO.obj.listing.columns_number; i++){
			GO.obj.listing.columns[i+1] = 0;
		}
		GO.obj.listing.current_column = 1;
	}

	var special_margin = GO.obj.listing.columns_number == 1 ? "-10px" : "";

	$("#tabbed-content-group").css({marginLeft: special_margin, marginRight: special_margin});

	$container.removeClass("small-cols").addClass(GO.obj.listing.columns_number > 6 ? "small-cols" : "");
	...
	$list_item.each(function(index) {
		...
		var width_responsive = GO.obj.listing.columns_number == 1 ? "100%" : parseInt((1/GO.obj.listing.columns_number)*($container.width() - (10 * (GO.obj.listing.columns_number - 1))) + "px");
		$(this).css("width", width_responsive);

		if(GO.obj.listing.current_column > GO.obj.listing.columns_number){
			GO.obj.listing.current_column = 1
		}

		$(this).attr("data-col", GO.obj.listing.current_column);
		...
		if(hard_forced && GO.obj.listing.columns_number > 1) {
		...
		if(empty || ($list_item_img.css("min-height") && !$list_item_src.hasClass("jsly"))) {
			...
			if(GO.obj.listing.columns_number > 6 && (list_item_src_pitfall_x > 0 || list_item_src_pitfall_y > 0)){
		...
		GO.obj.listing.columns[GO.obj.listing.current_column] += $(this).outerHeight(true);

		if(GO.obj.listing.columns_number == 1) {
			...
			var new_left = $(this).outerWidth(true)*(GO.obj.listing.current_column - 1);
			...
			var new_top = GO.obj.listing.columns[GO.obj.listing.current_column] - $(this).outerHeight(true);
		...
		if(!isFixed) {
			...
			for(var i=1; i<=GO.obj.listing.columns_number; i++){
				currentH = GO.obj.listing.columns[i];
				...
				if(GO.obj.listing.columns[i] == 0) {
					minCol = i;
					break;
				}
				if(currentH < minH) {
					minH = GO.obj.listing.columns[i];
					minCol = i;
				}
			}

			GO.obj.listing.current_column = minCol;
		} else {
			GO.obj.listing.current_column++;
		}

	});

	$container.data({"columns": GO.obj.listing.columns, "current_column": GO.obj.listing.current_column});

	var content_listing_height = 0;
	$.each(GO.obj.listing.columns, function(i, v){
	...
	GO.obj.listing.width = $container.width();

	if(typeof GO.obj.listing.height !== typeof undefined) {
		var old_listing_height = GO.obj.listing.height;
	}
	GO.obj.listing.height = content_listing_height;

	var do_listing_h_resize = typeof old_listing_height !== typeof undefined && old_listing_height !== GO.obj.listing.height;
	...
	$container.data("list-mode", GO.obj.listing.mode);
	$(GO.obj.listing.selectors.content_listing_visible).data("queued", false);
...
GO.fn.loading = {
	...
	inline: function($target, options) {
		...
		var size = GO.fn.loading.spin[options.size];

		GO.fn.loading.spin[options.size].color = options.color;
		GO.fn.loading.spin[options.size].shadow = options.shadow;

		$target.html('<span class="loading-indicator"></span>' + (typeof options.message !== "undefined" ? '<span class="loading-text">'+ options.message +'</span>' : '')).css({"line-height": GO.fn.loading.spin[options.size].blocksize + "px"});

		$(".loading-indicator", $target).css({width: GO.fn.loading.spin[options.size].blocksize, height: GO.fn.loading.spin[options.size].blocksize}).spin(GO.fn.loading.spin[options.size]);
		...
		if(options.center){
			$(".loading-indicator", $target.css("textAlign", "center")).css({
				...
				top: "50%",
				left: "50%",
				marginTop: -(GO.fn.loading.spin[options.size].blocksize/2),
				marginLeft: -(GO.fn.loading.spin[options.size].blocksize/2)
			});
		}
		if(options.valign == "center") {
			$(".loading-indicator,.loading-text", $target).css("marginTop", ($target.height()-GO.fn.loading.spin[options.size].blocksize)/2 + "px");
		}

		$(".spinner", $target).css({top: GO.fn.loading.spin[options.size].blocksize/2 + "px", left: GO.fn.loading.spin[options.size].blocksize/2 + "px"});

	},
	fullscreen: function(){
		$("body").append('<div class="fullscreen" id="pf-fullscreen-loader"><div class="fullscreen-loader black-bkg"><span class="loading-txt">' + GO.fn._s("loading") + '</span></div></div>');
		$(".fullscreen-loader", "#pf-fullscreen-loader").spin(GO.fn.loading.spin.huge);
...
GO.obj.follow_scroll = {
	...
	set: function(reset) {
		if(reset) {
			GO.obj.follow_scroll.base_h = $(".follow-scroll").outerHeight();
		}
		var exists = GO.obj.follow_scroll.$node.closest(".follow-scroll-wrapper").exists();
		if(exists) {
			GO.obj.follow_scroll.$node.closest(".follow-scroll-wrapper").css("position", "static");
		}
		GO.obj.follow_scroll.y = GO.obj.follow_scroll.$node.exists() ? GO.obj.follow_scroll.$node.offset().top : null;
		GO.obj.follow_scroll.node_h = GO.obj.follow_scroll.$node.outerHeight();
		if(exists) {
			GO.obj.follow_scroll.$node.closest(".follow-scroll-wrapper").css("position", "");
		}
	},
...
GO.obj.follow_scroll.set();
//GO.obj.follow_scroll.checkDocumentHeight();

GO.obj.follow_scroll.process = function(forced) {

	if(forced) {
		GO.obj.follow_scroll.node_h = GO.obj.follow_scroll.base_h;
	}

	if(!GO.obj.follow_scroll.$node.exists()) return; // Nothing to do here

	var $parent = GO.obj.follow_scroll.$node.closest("[data-content=follow-scroll-parent]");
	if(!$parent.exists()) {
		$parent = GO.obj.follow_scroll.$node.closest(".content-width");
	}

	var $wrapper = GO.obj.follow_scroll.$node.closest('.follow-scroll-wrapper');

	var top = GO.obj.follow_scroll.node_h;
	var	cond = $(window).scrollTop() > GO.obj.follow_scroll.y - top;

	if($("#top-bar").css("position") !== "fixed") {
		GO.obj.follow_scroll.Y -= $(window).scrollTop();
		if(GO.obj.follow_scroll.Y < 0) GO.obj.follow_scroll.Y = 0;
		cond = cond && $(window).scrollTop() > GO.obj.follow_scroll.y;
	}
	...
	if(!$wrapper.exists()) {
		GO.obj.follow_scroll.$node.wrapAll('<div class="follow-scroll-wrapper" />');
		$wrapper = GO.obj.follow_scroll.$node.closest('.follow-scroll-wrapper');
	}

	$wrapper.css("min-height", GO.obj.follow_scroll.node_h);

	GO.obj.follow_scroll.Y = $("#top-bar").outerHeight(true) + parseFloat($("#top-bar").css("top"));

	if(cond) {
		var placeholderHeight = GO.obj.follow_scroll.node_h;
		$wrapper
			//.addClass("position-fixed")
			.css({top: GO.obj.follow_scroll.Y});
	...
	GO.obj.follow_scroll.$node[(cond ? "add" : "remove") + "Class"]("content-width");

};

GO.fn.isDevice = function(device) {
...
GO.fn.getDeviceName = function() {
	var current_device;
	$.each(GO.obj.devices, function(i,v) {
		if(GO.fn.isDevice(v)) {
			current_device = v;
			return true;
		}
...
GO.fn.topMenu = {
	vars : {
		...
		speed: GO.obj.config.animation.fast,
